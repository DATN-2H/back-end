...
public class RestExceptionHandler {
    ...
    private void writeLog(int code, String message, Exception exception) {
        ...
        logger.error(String.format(logFormat,
                code,
                message,
    ...
    public void handleUsernameNotFoundException(Exception exception, HttpServletResponse response) throws IOException {
        writeLog(ApiMessageBase.UNAUTHORIZED, exception);
        Response.servletResponse(response, ApiMessageBase.UNAUTHORIZED);
    }

    @ExceptionHandler(AccessDeniedException.class)
    public void handleAccessDeniedException(AccessDeniedException exception, HttpServletResponse response) throws IOException {
    ...
    public void handleHttpMediaTypeException(Exception exception, HttpServletResponse response) throws IOException {
    ...
    public void handleMethodArgumentNotValidException(MethodArgumentNotValidException exception, HttpServletResponse response) throws IOException {
        ...
        if (!fieldErrorList.isEmpty()) {
            for (FieldError fieldError : fieldErrorList) {
                ...
                Response.servletResponse(response, ApiMessageBase.INVALID_PARAM.getCode(), message);
                return;
            }
        }
    ...
    public void handleInvalidFormatException(HttpServletResponse response, InvalidFormatException ex) throws IOException {
        writeLog(ApiMessageBase.NOT_READABLE_PARAM, ex);

        if (ex.getPath() != null && !ex.getPath().isEmpty()) {
            String message = ex.getPath().get(0).getFieldName() + " \"" + ex.getValue() + "\" is invalid";
            Response.servletResponse(response, ApiMessageBase.NOT_READABLE_PARAM.getCode(), message);
        } else {
    ...
    public void handleHttpMessageNotReadableException(HttpMessageNotReadableException exception, HttpServletResponse response) throws IOException {
        if (exception.getCause() instanceof InvalidFormatException) {
            handleInvalidFormatException(response, (InvalidFormatException) exception.getCause());
        } else {
    ...
    public void handleRestTemplateException(RestTemplateException exception, HttpServletResponse response) throws IOException {
        writeLog(exception);
        Response.servletResponse(response, exception.getCode(), exception.getMessage());
    }
    ...
    public void handleMissingParam(MissingServletRequestParameterException exception, HttpServletResponse response) throws IOException {
        writeLog(ApiMessageBase.MISSING_PARAM, exception);
        String message = exception.getParameterName();
        Response.servletResponse(response, ApiMessageBase.MISSING_PARAM.getCode(), message + " param is missing");
    }
    ...
    public void handleUploadSizeExceeded(MaxUploadSizeExceededException exception, HttpServletResponse response) throws IOException {
    ...
    public void handleTypeMismatchException(MethodArgumentTypeMismatchException exception, HttpServletResponse response) throws IOException {
        writeLog(ApiMessageBase.INVALID_PARAM, exception);
        String message = exception.getName() + " \"" + exception.getValue() + "\" is invalid";
        Response.servletResponse(response, ApiMessageBase.INVALID_PARAM.getCode(), message);
    }

    @ExceptionHandler(HttpRequestMethodNotSupportedException.class)
    public void handleTypeMismatchException(HttpRequestMethodNotSupportedException exception, HttpServletResponse response) throws IOException {
        writeLog(ApiMessageBase.REQUEST_METHOD_NOT_SUPPORT, exception);
        String message = exception.getMessage();
        Response.servletResponse(response, ApiMessageBase.REQUEST_METHOD_NOT_SUPPORT.getCode(), message);
    ...
    public void handleDataIntegrityViolationException(DataIntegrityViolationException exception, HttpServletResponse response) throws IOException {
        if (exception.getCause() instanceof ConstraintViolationException) {
            handleConstraintViolationException((ConstraintViolationException) exception.getCause(), response);
            ...
            Response.servletResponse(response, ApiMessageBase.DATA_INTEGRITY_VIOLATION.getCode(), exception.getMessage());
        }
    }
    ...
    public void handleConstraintViolationException(ConstraintViolationException exception, HttpServletResponse response) throws IOException {
        ...
        var code = StringUtils.substring(detailMessage, detailMessage.indexOf("=(") + 2, detailMessage.indexOf("already exists") - 2); // DETACH CODE UNIQUE

        writeLog(ApiMessageBase.DATA_INTEGRITY_VIOLATION, exception);
        Response.servletResponse(response, ApiMessageBase.DATA_INTEGRITY_VIOLATION.getCode(), ApiMessageBase.DATA_INTEGRITY_VIOLATION + exception.getConstraintName());
    }

    @ExceptionHandler(DataException.class)
    public void handleDataException(DataException exception, HttpServletResponse response) throws IOException {
    ...
    public void handlePSQLException(PSQLException exception, HttpServletResponse response) throws IOException {
        writeLog(ApiMessageBase.DATA_INTEGRITY_VIOLATION, exception);
        Response.servletResponse(response, ApiMessageBase.DATA_INTEGRITY_VIOLATION.getCode(), STR."data_integrity_violate.\{exception.getSQLState()}");
    }
    ...
    public void handleUnknownException(Exception exception, HttpServletResponse response) throws IOException {
