...
public class PermissionValidator {
    ...
    public boolean hasPermission(Authentication authentication, HttpServletRequest request) {
        ...
        if (null == userCustom) {
            log.error("PermissionValidator: don't have permission url={}, method={}", request.getServletPath().replaceAll("[\\n]", ""),
                    request.getMethod());
            return false;
        }
        ...
        List<PermissionEvaluateDto> permissionEvaluateDtos = userService.getPermissionEvaluate(userCustom.getUserId());
        ...
        String pattern = requestMappingInfo.getPatternsCondition().getPatterns().stream().findFirst().get();

        PermissionEvaluateDto matchedPermissionEvaluate = permissionEvaluateDtos.stream()
                .filter(pv -> pv.getMethod() == method && pv.getPattern().equals(pattern)).findFirst().orElse(null);

        if (matchedPermissionEvaluate == null) {
            log.error("PermissionValidator: don't have match permission url={}, method={}", request.getServletPath().replaceAll("[\\n]", ""),
                    request.getMethod());
            return false;
        }
