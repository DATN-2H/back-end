...
public class QueryUtil {
    ...
    private static <T> CriteriaQuery<Long> createCountQuery(
        final CriteriaBuilder cb,
        final CriteriaQuery<T> criteria,
        final Root<T> root
    ) {
    ...
    private static void doJoinsOnFetches(
        Set<? extends Fetch<?, ?>> joins,
        Root<?> root
    ) {
    ...
    private static void doJoins(
        Set<? extends Join<?, ?>> joins,
        Join<?, ?> root
    ) {
        for (Join<?, ?> join : joins) {
            Join<?, ?> joined = root.join(
    ...
    public static Join<?, ?> getOrJoin(
        From<?, ?> from,
        List<Pair<String, JoinType>> attributesPair
    ) {
        ...
        if (matchedJoin == null) {
            matchedJoin = from.join(
                currentAttribute,
                currentAttributePair.getSecond()
            );
        }
    ...
    public static Expression<String> concat(
        CriteriaBuilder cb,
        String separator,
    ...
    public static Expression<String> formatBooleanExpr(
        CriteriaBuilder builder,
        Expression<Boolean> expr
    ) {
        Expression<String> partialExpr = builder.function(
            Constant.REPLACE,
            String.class,
            builder.lower(expr.as(String.class)),
    ...
    public static <T> Expression<String> formatEnumExpr(
        CriteriaBuilder builder,
        Expression<T> expr
    ) {
        return builder.function(
            ...
            String.class,
            builder.lower(expr.as(String.class)),
            builder.literal(Constant.DASH),
            builder.literal(Constant.SPACE)
        );
    }

    public static Expression<String> formatDateExpr(
        CriteriaBuilder builder,
        Expression<LocalDate> expr
    ) {
        return builder.function(
            Constant.TO_CHAR,
            String.class,
            expr,
            builder.literal(Constant.DATE_FORMAT)
        );
    }

    public static Expression<String> formatDateTimeExpr(
        CriteriaBuilder builder,
        Expression<LocalDateTime> expr
    ) {
        return builder.function(
            Constant.TO_CHAR,
            String.class,
            expr,
            builder.literal(Constant.DATE_TIME_FORMAT)
        );
    }
}
