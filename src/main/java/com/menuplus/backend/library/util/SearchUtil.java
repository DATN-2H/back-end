...
public class SearchUtil {
    ...
    public static String getNativeSearchPredicate(SearchCondition condition) {
        String fieldName = NamingUtil.camelCaseToSnakeCase(
    ...
    private static String getStringPredicate(
        String fieldName,
        ...
        return switch (condition.getOperatorType()) {
            ...
            fieldName +
            ") LIKE CONCAT('%', '" +
            condition.getData().toLowerCase() +
            "', '%')";
            case START_WITH -> "LOWER(" +
            fieldName +
            ") LIKE CONCAT('" +
            condition.getData().toLowerCase() +
            "', '%')";
    ...
    private static String getNumericPredicate(
        String fieldName,
        SearchCondition condition
    ) {
        return switch (condition.getOperatorType()) {
            ...
            case BETWEEN -> fieldName +
            " BETWEEN " +
            condition.getDatas().get(0) +
            " AND " +
            condition.getDatas().get(1);
            default -> "1=1";
        };
    }

    private static String getBooleanPredicate(
        String fieldName,
        SearchCondition condition
    ) {
    ...
    private static String getDatePredicate(
        String fieldName,
        SearchCondition condition
    ) {
        return switch (condition.getOperatorType()) {
            case EQUAL -> fieldName + " = '" + condition.getData() + "'";
            case GREATER_EQUAL -> fieldName +
            " >= '" +
            condition.getData() +
            "'";
            case LESS_EQUAL -> fieldName + " <= '" + condition.getData() + "'";
    ...
    public static Predicate getSearchPredicate(
        ...
        return switch (condition.getOperandType()) {
            ...
            case DATETIME -> getDateTimePredicate(
                builder,
                expression,
                condition
            );
    ...
    private static Predicate getStringPredicate(
        CriteriaBuilder builder,
        Expression<?> expression,
        SearchCondition condition
    ) {
        return switch (condition.getOperatorType()) {
            case CONTAIN -> builder.like(
                builder.lower(expression.as(String.class)),
                format(condition.getData())
            );
            case START_WITH -> {
                String formatStartSearch =
                    condition.getData().toLowerCase() + "%";
                yield builder.like(
                    builder.lower(expression.as(String.class)),
                    formatStartSearch
    ...
    private static Predicate getEnumPredicate(
        CriteriaBuilder builder,
        Expression<?> expression,
        SearchCondition condition
    ) {
        String data = condition
            .getData()
            .trim()
            .replaceAll(Constant.BLANK_REGEX, Constant.DASH);
    ...
    private static Predicate getDateTimePredicate(
        CriteriaBuilder builder,
        Expression expression,
        SearchCondition condition
    ) {
        if (condition.getOperatorType() == OperatorType.BETWEEN) {
            return builder.and(
                builder.greaterThanOrEqualTo(
                    expression,
                    parseDateTime(condition.getDatas().get(0))
                ),
                builder.lessThanOrEqualTo(
                    expression,
                    parseDateTime(condition.getDatas().get(1))
                )
            );
        }
        return switch (condition.getOperatorType()) {
            case GREATER_EQUAL -> builder.greaterThanOrEqualTo(
                expression,
                parseDateTime(condition.getData())
            );
            case LESS_EQUAL -> builder.lessThanOrEqualTo(
                expression,
                parseDateTime(condition.getData())
            );
            case EQUAL -> builder.equal(
    ...
    private static Predicate getDatePredicate(
        CriteriaBuilder builder,
        Expression expression,
        SearchCondition condition
    ) {
        if (condition.getOperatorType() == OperatorType.BETWEEN) {
            return builder.and(
                builder.greaterThanOrEqualTo(
                    expression,
                    parseDate(condition.getDatas().get(0))
                ),
                builder.lessThanOrEqualTo(
                    expression,
                    parseDate(condition.getDatas().get(1))
                )
            );
        }
        return switch (condition.getOperatorType()) {
    ...
    private static Predicate getBoolPredicate(
        CriteriaBuilder builder,
        Expression expression,
        SearchCondition condition
    ) {
    ...
    private static Predicate getDecimalPredicate(
        CriteriaBuilder builder,
        Expression expression,
        SearchCondition condition
    ) {
        if (condition.getOperatorType() == OperatorType.BETWEEN) {
            return builder.and(
                builder.greaterThanOrEqualTo(
                    expression,
                    parseDecimal(condition.getDatas().get(0))
                ),
                builder.lessThanOrEqualTo(
                    expression,
                    parseDecimal(condition.getDatas().get(1))
                )
            );
        }
        ...
        return switch (condition.getOperatorType()) {
            case GREATER_EQUAL -> builder.greaterThanOrEqualTo(
                expression,
                parseDecimal(condition.getData())
            );
            case LESS_EQUAL -> builder.lessThanOrEqualTo(
                expression,
                parseDecimal(condition.getData())
    ...
    private static Predicate getIntegerPredicate(
        CriteriaBuilder builder,
        Expression expression,
        SearchCondition condition
    ) {
        if (condition.getOperatorType() == OperatorType.BETWEEN) {
            return builder.and(
                builder.greaterThanOrEqualTo(
                    expression,
                    parseInteger(condition.getDatas().get(0))
                ),
                builder.lessThanOrEqualTo(
                    expression,
                    parseInteger(condition.getDatas().get(1))
                )
            );
        }
        ...
        return switch (condition.getOperatorType()) {
            case GREATER_EQUAL -> builder.greaterThanOrEqualTo(
                expression,
                parseInteger(condition.getData())
            );
            case LESS_EQUAL -> builder.lessThanOrEqualTo(
                expression,
                parseInteger(condition.getData())
