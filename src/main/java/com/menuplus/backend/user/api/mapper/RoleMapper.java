...
public class RoleMapper {
    public static List<String> ListExcludeFields = List.of(Role_.ROLE_PERMISSIONS, Role_.ROLE_SCREENS);
    ...
    public static RoleResponseDto createResponse(Role entity, List<String> excludeFields) {
        RoleResponseDto responseDto = new RoleResponseDto();
        MapUtil.copyResponseProperties(entity, responseDto);
        if (!excludeFields.contains(Role_.ROLE_PERMISSIONS) && entity.getRolePermissions() != null) {
            responseDto.setRolePermissions(MapperHelper.map(new ArrayList<>(entity.getRolePermissions()),
                    RolePermissionMapper::createResponse));
        }
        if (!excludeFields.contains(Role_.ROLE_SCREENS) && entity.getRoleScreens() != null) {
            responseDto.setRoleScreens(MapperHelper.map(new ArrayList<>(entity.getRoleScreens()),
