...
public class UserMapper {
    ...
    public static List<String> PermissionExcludeFields = List.of(User_.USER_ROLES);
    ...
    public static UserDtoResponse createResponse(User entity, List<String> excludeFields) {
        UserDtoResponse response = new UserDtoResponse();
        MapUtil.copyResponseProperties(entity, response);

        if (!excludeFields.contains(User_.USER_ROLES) && entity.getUserRoles() != null) {
            response.setUserRoles(MapperHelper.map(new ArrayList<>(entity.getUserRoles()),
