...
public class UserCurrentController {
    ...
    public Response<UserClaims> verifyToken(@Valid @RequestBody VerifyTokenRequest request) {
        return Response.success(userService.verifyToken(request));
    }

    @PostMapping(value = "/forget-password", consumes = Constant.API_CONTENT_TYPE)
    public Response<Void> forgetPassword(@Valid @RequestBody UserForgetPasswordRequest request) {
        userService.forgetPassword(request);
        return Response.success();
    }

    @PutMapping(value = "/change-password", consumes = Constant.API_CONTENT_TYPE)
    public Response<Void> changePassword(@Valid @RequestBody UserChangePasswordRequest request) {
