...
public class RolePermissionServiceImpl implements RolePermissionService {
    ...
    @Override
    @Transactional
    public List<RolePermissionResponseDto> createMany(List<RolePermissionCreateDto> createDtos) {
        ...
        return MapperHelper.map(rolePermissions, RolePermissionMapper::createResponse);
    }

    @Override
    @Transactional
    public List<RolePermissionResponseDto> upsert(Long roleId, List<RolePermissionCreateDto> upsertDtos) {
        List<Long> upsertPermissionIds = upsertDtos.stream().map(RolePermissionCreateDto::getPermissionId).toList();

        List<RolePermission> existingRolePermissions = rolePermissionRepository.findByRoleId(roleId);
        List<Long> existingPermissionIds = existingRolePermissions.stream().map(RolePermission::getPermissionId).toList();

        Map<Boolean, List<RolePermission>> existingMap = existingRolePermissions.stream().collect(
                Collectors.partitioningBy(rolePermission -> upsertPermissionIds.contains(rolePermission.getPermissionId())));
        ...
        Map<Boolean, List<RolePermissionCreateDto>> requestMap = upsertDtos.stream()
                .collect(Collectors.partitioningBy(c -> existingPermissionIds.contains(c.getPermissionId())));
        List<RolePermissionCreateDto> createDtos = requestMap.get(false);
        List<RolePermissionCreateDto> updateDtos = requestMap.get(true);

        System.out.printf("RolePermissionService: existing permissionIds: %s",
                Arrays.toString(Arrays.stream(existingPermissionIds.toArray()).sorted().toArray()));

        System.out.printf("RolePermissionService: update permissionIds: %s",
                Arrays.toString(Arrays.stream(upsertPermissionIds.toArray()).sorted().toArray()));
    ...
    private List<RolePermissionResponseDto> updateMany(List<RolePermission> updateRoleScreens,
                                                       List<RolePermissionCreateDto> updateDtos) {
        Map<Long, RolePermissionCreateDto> updateDtoMap = updateDtos.stream().
        ...
        for (RolePermission updateRolePermission : updateRoleScreens) {
            var updateDto = updateDtoMap.get(updateRolePermission.getPermissionId());
            if (updateDto == null) {
                System.out.printf("RolePermissionService: ignore update permissionId: %d", updateRolePermission.getPermissionId());
                continue;
            }
            MapUtil.copyUpdateProperties(updateDto, updateRolePermission);
            rolePermissionRepository.save(updateRolePermission);
        }
        return MapperHelper.map(updateRoleScreens, RolePermissionMapper::createResponse);
    }
}
