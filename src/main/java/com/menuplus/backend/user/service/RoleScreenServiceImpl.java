...
public class RoleScreenServiceImpl implements RoleScreenService {
    ...
    @Override
    @Transactional
    public List<RoleScreenResponseDto> createMany(List<RoleScreenCreateDto> createDtos) {
    ...
    @Override
    @Transactional
    public List<RoleScreenResponseDto> upsert(Long roleId, List<RoleScreenCreateDto> upsertDtos) {
        List<Long> upsertIds = upsertDtos.stream().map(RoleScreenCreateDto::getId).
                filter(Objects::nonNull).toList();

        List<RoleScreen> existingRoleScreens = roleScreenRepository.findByRoleId(roleId);
        Map<Boolean, List<RoleScreen>> existingMap = existingRoleScreens.stream().collect(
                Collectors.partitioningBy(roleScreen -> upsertIds.contains(roleScreen.getId())));
        List<RoleScreen> updateRoleScreens = existingMap.get(true);
        List<RoleScreen> deleteRoleScreens = existingMap.get(false);

        Map<Boolean, List<RoleScreenCreateDto>> requestMap = upsertDtos.stream()
    ...
    private List<RoleScreenResponseDto> updateMany(List<RoleScreen> updateRoleScreens,
                                                   List<RoleScreenCreateDto> updateDtos) {
        Map<Long, RoleScreenCreateDto> updateDtoMap = updateDtos.stream().
                collect(Collectors.toMap(RoleScreenCreateDto::getId, Function.identity()));
        ...
        return MapperHelper.map(updateRoleScreens, RoleScreenMapper::createResponse);
    }
}
