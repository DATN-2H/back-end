...
public class UserRoleServiceImpl implements UserRoleService {
    ...
    @Transactional
    @Override
    public List<UserRoleResponseDto> createMany(List<UserRoleCreateDto> createDtos) {
    ...
    @Override
    @Transactional
    public List<UserRoleResponseDto> upsert(Long userId, List<UserRoleCreateDto> upsertDtos) {
        List<Long> upsertIds = upsertDtos.stream().map(UserRoleCreateDto::getId).
                filter(Objects::nonNull).toList();

        List<UserRole> existingUserRoles = userRoleRepository.findByUserId(userId);
        Map<Boolean, List<UserRole>> existingMap = existingUserRoles.stream().collect(
                Collectors.partitioningBy(userRole -> upsertIds.contains(userRole.getId())));
        List<UserRole> updateUserRoles = existingMap.get(true);
        List<UserRole> deleteUserRoles = existingMap.get(false);

        Map<Boolean, List<UserRoleCreateDto>> requestMap = upsertDtos.stream()
    ...
    private List<UserRoleResponseDto> updateMany(List<UserRole> updateUserScreens,
                                                 List<UserRoleCreateDto> updateDtos) {
        Map<Long, UserRoleCreateDto> updateDtoMap = updateDtos.stream().
